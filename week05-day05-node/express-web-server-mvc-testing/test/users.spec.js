...
// TEST FOR UPDATE
  ...
  describe('POST', function(){
    it('should return ERROR when firstName is blank', function(done){
      ...
      .post('/users')
      .set('Content-Type', 'application/x-www-form-urlencoded')
      .send({firstName: '',  email: 'testPostEmail@example.com'})
      .end(function (err, res){
    ...
    it.only('should create a new user when input is valid', function(done){
      var testFirstName = generateUniqueFirstName();

      request
      .post('/users/')
      .set('Content-Type', 'application/x-www-form-urlencoded')
      .send({firstName: testFirstName,  email: 'testEmailPost'})
      .end(function(err,res){
